...
<div class="contentContainer <?php echo (OSCOM_APP_PAYPAL_LOGIN_CONTENT_WIDTH == 'Half') ? 'grid_8' : 'grid_16'; ?>">
  <h2><?php echo $cm_paypal_login->app->getDef('module_login_template_title'); ?></h2>

  <div class="contentText">

<?php
  if ( OSCOM_APP_PAYPAL_LOGIN_STATUS == '0' ) {
    echo '    <p class="messageStackError">' . $cm_paypal_login->app->getDef('module_login_template_sandbox_alert') . '</p>';
  }
?>

    <p><?php echo $cm_paypal_login->app->getDef('module_login_template_content'); ?></p>
...
?>
    ...
    "locale": "<?php echo $cm_paypal_login->app->getDef('module_login_language_locale'); ?>",
    ...
    "returnurl": "<?php echo str_replace('&amp;', '&', OSCOM::link('login.php', 'action=paypal_login', false)); ?>"
  });
});
</script>
